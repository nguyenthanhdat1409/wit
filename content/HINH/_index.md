...
<div class="search-container">
  <div class="search-wrapper">
    <input 
      ...
      id="searchInput" 
      class="search-input" 
      placeholder="Tìm kiếm hình ảnh - đồ hình..."
      autocomplete="off"
    />
...
function initializeHinhSearch() {
  ...
  searchButton.addEventListener('click', function() {
    performHinhSearch(searchInput.value.trim());
    searchInput.focus();
  });
  ...
  updateHinhSearchInfo(allHinhItems.length, allHinhItems.length);
}

// Perform search
function performHinhSearch(searchTerm) {
  if (!searchTerm) {
    filteredHinhItems = allHinhItems;
    renderFilteredHinhCards(filteredHinhItems);
    updateHinhSearchInfo(allHinhItems.length, allHinhItems.length);
    return;
  }
  ...
  filteredHinhItems = allHinhItems.filter(item => {
    const title = (item.title || '').toLowerCase();
    const content = (item.content || '').replace(/<[^>]*>/g, '').toLowerCase();
    return title.includes(searchLower) || content.includes(searchLower);
  });
  
  renderFilteredHinhCards(filteredHinhItems, searchTerm);
  updateHinhSearchInfo(filteredHinhItems.length, allHinhItems.length);
}

// Update search info
function updateHinhSearchInfo(found, total) {
  const searchResultsInfo = document.getElementById('searchResultsInfo');
  const searchInput = document.getElementById('searchInput');
  const searchTerm = searchInput ? searchInput.value.trim() : '';
  
  if (!searchTerm) {
    searchResultsInfo.innerHTML = `📊 Hiển thị <strong>${total}</strong> hình ảnh - đồ hình`;
  } else if (found === 0) {
    searchResultsInfo.innerHTML = `❌ Không tìm thấy kết quả cho "<strong>${searchTerm}</strong>"`;
  } else {
    searchResultsInfo.innerHTML = `✅ Tìm thấy <strong>${found}</strong> kết quả từ <strong>${total}</strong> hình ảnh`;
  }
}

// Escape regex
function escapeRegexHinh(string) {
...
async function loadDoHinhData() {
  try {
    ...
    allHinhItems = items;
    filteredHinhItems = items;
    
    renderDoHinhCards(data);
    initializeHinhSearch();
  } catch (error) {
...
function renderDoHinhCards(data) {
  ...
  renderFilteredHinhCards(items);
}

// Render filtered cards with optional highlight
function renderFilteredHinhCards(items, searchTerm = '') {
  ...
  imageGrid.innerHTML = items.map(item => {
    ...
    if (searchTerm) {
      const regex = new RegExp(`(${escapeRegexHinh(searchTerm)})`, 'gi');
